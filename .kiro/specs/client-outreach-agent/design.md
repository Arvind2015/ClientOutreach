...
InboundMessage
- message_id, case_id (nullable pre-correlation)
- sender, received_at, raw_ref (S3)
- attachments: [ { attachment_id, s3_ref, classification, confidence,
...
| Low classification/extraction confidence | Flag `NEEDS_ANALYST_REVIEW`, halt automation for that item (Req 6.6, 12.1) |
...
- **Agent evaluation:** golden-set test emails/attachments (including edge cases: expired docs, wrong document type, low-quality scans, non-English content, malicious file types) run against the Inbound Analysis Agent with confidence-threshold assertions.
