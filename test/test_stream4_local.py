...
def build_test_email():
    """Create a minimal MIME email with one PDF attachment."""
    ...
    # Text body
    body = MIMEText("Please find attached my certificate of incorporation.", "plain")
    msg.attach(body)

    # Fake PDF attachment (2 KB of dummy bytes — above MIN_FILE_SIZE_BYTES)
    fake_pdf = b"%PDF-1.4 " + b"x" * 2048
    attachment = MIMEApplication(fake_pdf, _subtype="pdf")
    attachment.add_header("Content-Disposition", "attachment", filename="cert_of_inc.pdf")
...
def main():
    ...
    if classify_result.get("classified_attachments"):
        att = classify_result["classified_attachments"][0]
        print(f"  Classification:    {att['classification']} (confidence: {att['classification_confidence']})")
    if extract_result.get("extracted_attachments"):
        att = extract_result["extracted_attachments"][0]
        print(f"  Extraction:        {len(att.get('extracted_fields', {}))} fields (confidence: {att['extraction_confidence']})")
